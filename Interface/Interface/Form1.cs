...
namespace Interface
{
    ...
    public partial class MainForm : Form
    {
        ...
        private void num_r_CheckedChanged(object sender, EventArgs e)
        {
            if (t_num)
            {
                t_num = false;
        ...
        private void arifmetical_b_CheckedChanged(object sender, EventArgs e)
        {
            if (t_arif)
                t_arif = false;
            else
        ...
        public void LoadJson(string fs)
        {
        ...
        int komplex_ind(int t)//парсер комплексного числа
        {
            int i = t;
            while (i != -1&&input_textBox.Text[i]!='+' && input_textBox.Text[i] != '-' )
            {
                i--;
            }
            if (i == -1)
            ...
            else
            {
                ...
                else
                {
                    if (t_num)
                    {
                        //try
                        //{
                            a = new LongDouble(input_textBox.Text,9);
                    ...
                    else
                    {
                        try
                        {
                            ...
                            a_d = Convert.ToDouble(a_s); //реальная
                            b_d = Convert.ToDouble(b_s); //мнимая
       ...
       public class LongDouble
        {
            ...
            {
                if (digits.Count == 1 && digits[0] == 0)
                    throw new Exception("LongDouble inverse() - division by zero!"); // делить на ноль нельзя, поэтому бросим исключение
            ...
            public string analytical_sqrt()
            {
                ...
                {
                    throw new Exception($"Could not find analytical sqrt of {this.ToString()}");
                }
                if (r < 1)
                    return r.ToString();
        ...
        //        if (digits.Count == 1 && digits[0] == 0)
        //        {
        //            MessageBox.Show(texts.exception2);
        //            return null;
        //        }
        //         throw new Exception("LongDouble inverse() - division by zero!"); // делить на ноль нельзя, поэтому бросим исключение
        //        LongDouble x = new LongDouble(this); // скопируем число,
        ...
        //        if (!BigInteger.TryParse(this.ToString(), out r))
        //        {
        //            MessageBox.Show(texts.exception3);
        //            return "";
        //            //throw new Exception($"Could not find analytical sqrt of {this.ToString()}");
        //        }
        //        if (r < 1)
